...
export const VehiclesList: React.FC = () => {
  ...
  const fetchDependencies = async () => {
    try {
      const [custRes, agentRes, branchRes] = await Promise.all([
  ...
  useEffect(() => {
    fetchVehicles();
  }, [search, statusFilter, agentFilter, branchFilter, page]);
  ...
  const openEditModal = (vehicle: Vehicle) => {
    ...
    setFormAgentId(vehicle.agent_id);
  ...
  const handleCreateSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formVin || formVin.length !== 17 || !formAgentId) {
  ...
  const handleToggleStatus = async (vehicle: Vehicle) => {
    const newStatus = vehicle.status === 'active' ? 'archived' : 'active';
    const confirmMsg = newStatus === 'archived'
      ? 'هل تريد أرشفة ملف هذه السيارة؟'
      : 'هل تريد إلغاء الأرشفة وتنشيط السيارة؟';
    ...
    try {
      ...
      alert(err.messageAr || 'فشل تعديل حالة ملف السيارة.');
    }
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (row: Vehicle) => (
        <span>{row.agent?.full_name_ar || row.agent?.full_name || '-'}</span>
      )
    },
    {
      key: 'current_stage',
    ...
    },
    {
      ...
      render: (row: Vehicle) => (
        <div className="flex items-center justify-center gap-2" onClick={(e) => e.stopPropagation()}>
          <button
            onClick={() => navigate(`/admin/vehicles/${row.id}`)}
          ...
          {canWrite && (
            <>
              ...
              <button
                ...
                className="px-2.5 py-1 text-xs font-bold text-text-secondary border border-border hover:bg-bg-light rounded transition-all"
              >
                {row.status === 'active' ? 'أرشفة' : 'نشط'}
              </button>
            </>
          )}
        </div>
    ...
    <div className="flex flex-col gap-6 select-none">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 select-none">
        <div className="flex flex-col gap-1 text-right">
          <h1 className="text-2xl font-bold text-primary-dark">{ar.nav.vehicles}</h1>
          <p className="text-xs text-text-secondary">متابعة وإدارة ملفات السيارات المستوردة من الموانئ الأمريكية واللوجستيات لشركة الباشا</p>
        </div>
      ...
      {/* Filter panel */}
      <div className="bg-white p-4 rounded-lg border border-border shadow-sm grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
        <div className="text-right">
          <label className="text-[10px] text-text-secondary block mb-1 font-bold">رقم الهيكل VIN / اللوط</label>
          <Input
        ...
        <div className="text-right">
          <label className="text-[10px] text-text-secondary block mb-1 font-bold">{ar.vehicles.agent}</label>
          <select
            value={agentFilter}
            onChange={(e) => {
          ...
          >
            ...
            {agents.map(a => (
              <option key={a.id} value={a.id}>{a.full_name_ar}</option>
            ))}
          </select>
        </div>

        <div className="text-right">
          <label className="text-[10px] text-text-secondary block mb-1 font-bold">{ar.vehicles.branch}</label>
          <select
            value={branchFilter}
            onChange={(e) => {
      ...
      {errorMsg ? (
        ...
        <DataTable
          ...
          emptyDescription="لم نعثر على أي مركبات مطابقة للمحددات الحالية."
        />
      )}
      ...
      >
        <form onSubmit={handleCreateSubmit} className="flex flex-col gap-4 text-right">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div className="text-right">
              <label className="text-xs font-bold text-text-primary block mb-1">{ar.vehicles.agent}</label>
              <select
                value={formAgentId}
